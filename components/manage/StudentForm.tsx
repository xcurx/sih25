...
export function StudentForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setFeedback({ type: "success", message: data.message || "Student created successfully." })
        setForm({ name: "", email: "", password: "" })
      } else {
        setFeedback({ type: "error", message: data.error || "Failed to create student." })
      }
    } catch {
    ...
    <Card className="border-slate-200 bg-white/95 shadow-lg">
      <CardHeader>
        <div className="flex flex-col gap-3">
          ...
          <div>
            <CardTitle className="text-2xl text-slate-900">Add new student</CardTitle>
